...
class TextField:
    ...
    def draw(self, surface, fonts):
        ...
        border_color = COLORS["blue"] if self.active else (200, 200, 210)
        pygame.draw.rect(surface, border_color, self.rect, 2, border_radius=5)
        
        # Texto
        display_text = self.text if self.text else self.placeholder
        color = COLORS["text_dark"] if self.text else (150, 150, 150)
        text_surf = fonts["small"].render(display_text, True, color)
    ...
    def handle_event(self, event, fonts):
        ...
        if self.active and event.type == pygame.KEYDOWN:
            ...
            elif event.unicode.isdigit():  # Solo aceptar números
